...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="GxControl">
    <xsl:choose>
      <xsl:when test="@type = 'HugaoFileUpload'">
        <xsl:call-template name="RenderHugaoFileUpload"/>
      </xsl:when>
    </xsl:choose>
  </xsl:template>

  <!-- HugaoFileUpload design render -->
  <!-- ///////////////////  Implement your render here  ///////////////////-->
  <xsl:template name="RenderHugaoFileUpload">
    <span atomicselection="true">
      <xsl:call-template name="AddStyleAttribute"/>
      	<img width="48" height="48">
			<xsl:attribute name="src">
				<xsl:value-of select='gxca:GetMyPath()'/>
				<xsl:text>\HugaoUploadFile.png</xsl:text>
			</xsl:attribute>
			<xsl:call-template name="AddStyleAttribute"/>
      	</img>
    </span>
